...
- ず: だから、ゲームにおいて起こることをMessageで表現して、tsuzuriに投げるだけでOKなのだ！
- 画: シナリオ管理のソースコードを表示
- ず: 例えば、ゲームのシナリオはこんな形で管理できるのだ。
- め: なるほど、Messageの配列でシナリオを作ることができるのね。
- め: 一番最初のAddWidgetでテキストボックスを作成して、そこにテキストを追加しているみたいね。
- ず: そして作ったシナリオの配列は、クリックに合わせて1つずつtsuzuriに投げれば、シナリオは進んでいくのだ。
- め: なるほど。確かにやっていることはかなりシンプルね。
...
- ず: だから、自分でやらないといけないことは多いけど、逆にtsuzuriから強いる制約は少ないので、シナリオからUIまでかなり自由に設計できると思うのだ。
- め: なるほど。使いこなすにはそれなりに知識や技術力が求められるけど、逆にあるなら自由にゲーム制作ができる、という感じね。
- ず: そういうことなのだ。
- ず: だから、万人におすすめできるノベルゲームエンジンでは全くないけれど、プログラミングの経験がある人にはかなり刺さると思うのだ！
- ア: ゲーム画面フェードアウト・フェードイン
- ず: tsuzuriのアーキテクチャを実現するために、他にもUIライブラリ非依存の状態管理ライブラリを自作したり、オーディオ周りの宣言的な管理方法とかの、かなり尖ったこともやっているのだ。
